...
export interface ICapturePieceUseCase {
  gameId: string;
  from: Coordinates;
  to: Coordinates;
}
...
export class CapturePieceUseCase {
  ...
  public execute({ from, gameId, to }: ICapturePieceUseCase) {
    const playerId = this.socket.clientId;
    const gameRoom = this.orchestrator.get(gameId);

    if (!gameRoom) {
      logger.error(`CapturePieceUseCase: GameRoom not found`);
